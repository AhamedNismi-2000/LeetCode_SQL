...
Note: days_as_subscriber is calculated from the first event date to the last event date for each user.3716. Find Churn Risk Customers
Medium
premium lock icon
Companies
SQL Schema
...
-- Return the result table ordered by days_as_subscriber in descending order, then by user_id in ascending order.
    ...
    SELECT 
        ...
        le.current_monthly_amount,
        ch.max_historical_amount,
        DATEDIFF(ch.last_date, ch.first_date) AS days_as_subscriber   
    FROM latest_event le
    JOIN customer_history ch
        ON le.user_id = ch.user_id
    WHERE le.event_type != 'cancel'
        AND ch.subscription_history >= 1
        AND le.current_monthly_amount < ch.max_historical_amount * 0.5
        AND DATEDIFF(ch.last_date, ch.first_date) >= 60              
